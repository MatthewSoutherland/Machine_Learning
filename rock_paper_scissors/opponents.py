...
def play(player1, player2, num_games, verbose=False):
    ...
    with open("collect_data.csv", "w", newline="") as csvfile:
        pass  # erase data
    return win_rate
...
def human(prev_opponent_play):
    ...
    play = ""
    if prev_opponent_play != "":
        with open("collect_data.csv", "a", newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([prev_opponent_play])

            with open("collect_data.csv", "r") as csvfile:
    ...
    if count < 7:
        if count == 0:
            ...
            with open("collect_data.csv", "r") as f:
                reader = csv.reader(f)
                r_count = 0
                for row in reader:
                    row_str = "".join(row)
                    r_count += row_str.count("R")
            style = r_count
            f.close()

            with open("collect_data.csv", mode="r+") as file:
